...
coursesData = {
    "".join([char for char in faculty if char.isupper()])
    + str(courseId): {
...
studentData = {
    index: {
        ...
        "Course Registered": {
            courseId: {"Course Name": coursesData[courseId]["Course Name"]}
            for courseId in random.sample(
                list(coursesData.keys()), random.randint(1, 3)
            )
        },
    }
...
facultyData = {
    faculty: {
        "Students": {k: v for k, v in studentData.items() if v["Faculty"] == faculty},
        "Courses": {
            k: v
            for k, v in coursesData.items()
            if "".join([char for char in faculty if char.isupper()]) in k
        },
    }
    for faculty in facultyList
}
