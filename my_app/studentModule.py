...
def get_students(current_user):
    ...
    for student_id, student_info in studentData.items():
        ...
        if "Link" not in student_info:
            student_info["Link"] = url_for(
                "get_student", student_id=student_id, _external=True
            )
...
def get_student(student_id):
    ...
    if "Link" not in studentData[student_id]:
        studentData[student_id]["Link"] = url_for(
            "get_student", student_id=student_id, _external=True
        )
    ...
    return jsonify(
        {
            ...
            "links": url_for("get_student", student_id=student_id, _external=True),
        }
    )
...
def get_students_courses(student_id):
    ...
    return jsonify(
        {
            "courses": studentData[student_id]["Course Registered"],
            "links": url_for(
                "get_students_courses", student_id=student_id, _external=True
            ),
        }
    )
    ...
    if "Course Registered" in data:
        for c in list(data["Course Registered"].keys()):
            if c in studentData["Course Registered"]:
                continue
            if c in coursesData:
                studentData[student_id]["Course Registered"][c] = {
                    "Course Name": coursesData[c]["Course Name"]
                }
